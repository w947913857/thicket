...
function updateOrder(update){
	...
	if(state==1){  
		window.location.href='<%=path%>/carorder!updateOrder.action?carorder.state=0&carorder.oid='+oid;	  
	}else{   
	window.location.href='<%=path%>/carinfo!selCarOfid.action?carinfo.id='+cid;	  
	}
}
</script>
<script type="text/javascript">  
...
function getCount(){
	$.ajax({  
        type : "post",  
         url : "<%=path%>/carorder!selCarorderCount.action",  
         data: {"uid":$("#uid").val()},
         dataType: "json",
         async : false,   
...
function InitTable(index,cflag) { 
	...
	$.ajax({    
        type : "post",  
         url : "<%=path%>/carorder!selCarorderOfUid.action",    
