	...
	<tr>
		...
		<td width="100%" class="ver01" >
			<div class="box2" panelTitle="查询" showStatus="false">
				<form action="<%=path%>/carinfo!getcarinfoOfPager.action" id="queryForm" method="post">
				<table>
					<tr>
						<td>汽车标题：</td>
						<td>
							<input type="text" id="name" name="carinfo.cartitle"/>
							<input type="text" style="width:2px;display:none;"/>
						</td>
						<td>审核状态：</td>
						<td>
							<select name="carinfo.checkstatus" id="checktatus">
								<option value="3" selected="selected">选择所有</option>
...
<script type="text/javascript">
	...
	function initGrid() {
		grid = $("#dataBasic").quiGrid({
			columns:[
				{ display: '所属丛林闲居名称', name: 'clxjname', 	 align: 'left', width: "20%"},
			    { display: '标题', name: 'cartitle', 	 align: 'left', width: "20%"},
			    { display: '价格', name: 'price', align: 'left', width: "10%"},
			    { display: '汽车牌照', name: 'licence', 	 align: 'left',  wdith:"10%"} ,
			    { display: '审核状态', name: 'checkstatus', 	 align: 'left',  wdith:"10%"} ,
			    { display: '操作', isAllowHide: false, align: 'left', width:"20%",
		 ...
		 url: '<%=path%>/carinfo!getcarinfoOfPager.action?carinfo.joinid='+'${loginedUser.id}', sortName: 'id',rownumbers:true,checkbox:true,
	...
	function addUnit() {
		top.Dialog.open({
				URL:"<%=path%>/carinfo/carinfo-management-content.jsp",
				Title:"添加",Width:500,Height:350});     
	}
	//查看
	function onView(rowid){
		top.Dialog.open({
			URL:"<%=path%>/carinfo!getcarinfoOfId.action?carinfo.id=" + rowid,
