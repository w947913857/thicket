...
function updatePwd(){
	...
	diag.Title = "修改密码";
	diag.ID = "pwd";
	diag.URL="<%=path%>/system/layout/updatepwd.jsp"; 
	diag.Width=350; 
	diag.Height=230;            
	...
	diag.show();  
	diag.addButton("btn3","取消",function(){
		Dialog.getInstance('pwd').close();  
	});     
	diag.addButton("btn4","修改",function(){    
		top.document.getElementById("_DialogFrame_pwd").contentWindow.validateForm();  
	}); 
}  
function openLockWindow(){
	var diag = new top.Dialog();
		diag.Title = "系统锁屏";
		diag.ID = "a1";
		diag.URL="<%=path%>/system/layout/lock.jsp";
		diag.Width=350;
		diag.Height=150;
		diag.ShowCloseButton=false;
		diag.ShowCancelButton=false;
		...
		diag.addButton("btn1","注销系统",function(){
			top.Dialog.confirm("确定要退出系统吗",function(){window.location="<%=path%>/adminusers!logout.action"});
		})
		diag.addButton("btn2","确定解锁",function(){
			top.document.getElementById("_DialogFrame_a1").contentWindow.validateForm();
		})
		$.post("<%=path%>/adminusers!lockScreen.action");
}
	...
	<div id="bs_navright">
		<div class="bs_nav">
			...
			<div class="float_right padding_top2 padding_right5">
				...
				<span class="icon_attention hand" onclick="updatePwd()">修改密码</span> 
				<span class="icon_exit hand" onclick='top.Dialog.confirm("确定要退出系统吗",function(){window.location="<%=path%>/adminusers!logout.action"});'>退出系统</span>
			</div> 
			<div class="clear"></div>
		</div>
	</div>
