	...
	<tr>
		<td width="100%" class="ver01" >
			<div class="box2" panelTitle="查询" showStatus="false">
				<form action="<%=path%>/adminusers!getAdminUsersOfPager.action" id="queryForm" method="post">
				<table>
					<tr>
						<td>用户名：</td>
						<td>
							<input type="text" id="searchInput" name="adu.username"/>
...
<script type="text/javascript">
	...
	function initGrid() {
		grid = $("#dataBasic").quiGrid({
			columns:[
			    ...
			    { display: '真实姓名', name: 'name', 	 align: 'center',  width:"10%"} ,
			    { display: '用户类型', name: 'usertype', 	 align: 'center',  width:"10%"} ,
			    { display: '帐号有效性', name: 'userflag', 	 align: 'center',  width:"20%"} ,
		 ...
		 url: '<%=path%>/adminusers!getAdminUsersOfPager.action', sortName: 'id',rownumbers:true,checkbox:true,
	...
	function addUnit() {
		...
		top.Dialog.open({
				URL:"<%=path%>/adminuser/adminuser-management-content.jsp",
				Title:"添加",Width:500,Height:255});
	}
	//查看
	...
	function onEdit(rowid){
		top.Dialog.open({
			URL:"<%=path%>/adminuser/adminuser-management-contentAjax.jsp?id=" + rowid,
			Title:"修改",Width:500,Height:255});
	}
	//删除	
	function onDelete(rowid,rowidx){
		top.Dialog.confirm("确定要删除该记录吗？",function(){
		  	//删除记录
		  	$.post("<%=path%>/adminusers!deleteAdminUser.action",
		  			{"ids":rowid},
		  			function(result){
