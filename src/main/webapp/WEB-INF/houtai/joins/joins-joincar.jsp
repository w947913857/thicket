	...
	<tr>
		...
		<td width="100%" class="ver01" >
			<div class="box2" panelTitle="查询" showStatus="false">
				<form action="<%=path%>/joins!getJoinsOfPager.action" id="queryForm" method="post">
				<table>
					<tr>
						<td>车辆名称/型号：</td>
						<td> 
							<input type="text" id="name" name="joincar.carname"/>
							<input type="text" style="width:2px;display:none;"/>
						</td>
						<td>加盟进度：</td> 
						<td>  
							<select name="joincar.joinstatus" id="checktatus">
...
<script type="text/javascript">
	...
	function initGrid() {
		grid = $("#dataBasic").quiGrid({
			columns:[
				{ display: '车辆名称/型号', name: 'carname', 	 align: 'align', width: "10%"},
				{ display: '行驶证号', name: 'travelnumber', 	 align: 'align', width: "10%"},
				{ display: '车主姓名', name: 'theowner', 	 align: 'align', width: "10%"},
				{ display: '身份证号', name: 'idcard', 	 align: 'align', width: "10%"},
				{ display: '指定驾驶员姓名', name: 'drivername', 	 align: 'align', width: "10%"},
				{ display: '驾驶证号', name: 'drivernumber', 	 align: 'align', width: "10%"},
				{ display: '电话', name: 'driverphone', 	 align: 'align', width: "10%"},
				{ display: '加盟状态', name: 'joinstatus', 	 align: 'align', width: "10%"},
				{ display: '加盟时间', name: 'jointime', 	 align: 'align', width: "12%"},
		 ...
		 url: '<%=path%>/joincar!selJoincarOfPage.action', sortName: 'jointime',rownumbers:true,
	...
	function onView(rowid){
		top.Dialog.open({
			URL:"<%=path%>/joincar!getJoincarOfId.action?joincar.id=" + rowid,
			Title:"操作",Width:800,Height:440});  
	}
	//修改	 
	function onEdit(rowid){
		top.Dialog.open({
			URL:"<%=path%>/carinfo!getcarinfoOfId.action?isupdate=1&carinfo.id=" + rowid,
