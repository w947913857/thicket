...
//  Handling FWR events ------------------------------------------------------------------------------------------------
  ...
  window.fwr_event_handler = function fwr_event_handler() {
    ...
    switch (arguments[0]) {
      ...
      case "saved":
    	var returnflag ="解脱已经上传成功";
        name = arguments[1];
        var data = $.parseJSON(arguments[2]);
        if (data.saved) {
        	if(data.returnflag==1){
        		returnflag ="文字转语音解脱上传成功";
        	}else if(data.returnflag==2){
        		returnflag ="更改语音解脱上传成功";
        	}
          $('#upload_status').css({'color': '#0F0'}).text("语音上传成功"); 
          ...
          $('#upload_status').css({'color': '#F00'}).text("喔。。上传发生了意外，请重试");
        }
        break;
      ...
      case "save_failed":
        ...
        $uploadStatus.css({'color': '#F00'}).text("喔。。上传发生了意外，请重试");
        break;
