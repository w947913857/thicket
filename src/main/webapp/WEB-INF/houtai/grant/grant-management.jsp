	...
	<tr>
		...
		<td width="100%" class="ver01" >
			<div class="box2" panelTitle="查询" showStatus="false">
				<form action="<%=path%>/grant!getGrantOfPager.action" id="queryForm" method="post">
				<table>        
					<tr>
					    <td>捐款人：</td>
						<td>
							<input type="text" id="donationname" name="grant.donationname"/>
							<input type="text" style="width:2px;display:none;"/>
						</td>
						<td>受助人：</td>
						<td>
							<input type="text" id="recipients" name="grant.recipients"/>
							<input type="text" style="width:2px;display:none;"/>
						</td>
...
<script type="text/javascript">
	...
	function initGrid() {
		grid = $("#dataBasic").quiGrid({
			columns:[
				{ display: '捐款人', name: 'donationname', 	 align: 'center', width: "10%"},
			    { display: '物品', name: 'goods', 	 align: 'center', width: "15%"},
			    { display: '数量', name: 'num', align: 'center', width: "10%"},
			    { display: '价值（元）', name: 'worthless', align: 'center', width: "10%"},			   
			    { display: '使用去向', name: 'useto', align: 'center', width: "35%"},
			    { display: '受助人', name: 'recipients', align: 'center', width: "10%"},
			    { display: '捐赠时间', name: 'donationTime', align: 'center', width: "10%"},
		 ...
		 url: '<%=path%>/grant!getGrantOfPager.action', sortName: 'id',rownumbers:true,checkbox:true,
	...
	function onView(rowid){
		top.Dialog.open({
			URL:"<%=path%>/joins!getJoinsfoOfId.action?isupdate=2&joins.id=" + rowid,
	...
	function deleteUnit() {
		...
		top.Dialog.confirm("确定要删除吗？",function(){
			$.post("<%=path%>/grant!deleteGrant.action", 
					//获取所有选中行
					getSelectIds(grid),
					function(result){
						handleResult(result.status);
	...
	function showImportDialog(){
		top.Dialog.open({Title:"导入款物发放信息", 
						Message:"请上传excel文件", 
						URL:"<%=path%>/grant/uploadFile.jsp?type=2",
